...
;
; 本地副本说明：winget/choco 渠道的 Inno Setup 不附带非官方语言包，
; 故将其随仓库分发（installer/ChineseSimplified.isl），本地与 CI 均引用此副本。
; 本文件必须保存为 UTF-8 with BOM（Inno 6 以 BOM 识别 UTF-8，
; 否则按 LanguageCodePage=936 解析会导致中文乱码）。
...
DontCloseApplications=不要自动关闭应用程序(&D)
...
FinishedRestartMessage=为完成 %1 的安装，安装程序必须重新启动您的电脑。%n%n要立即重启吗？
